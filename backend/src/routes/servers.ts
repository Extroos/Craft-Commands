...
import { importService } from '../services/servers/ImportService';
...
router.post('/', requirePermission('server.create'), async (req, res) => {
    ...
    const newServer = {
        ...
        executable: config.executable || 'server.jar',
        status: 'OFFLINE'
    };
...
router.post('/:id/start', requirePermission('server.start'), async (req, res) => {
    ...
    if (server.status === 'INSTALLING') {
        return res.status(409).json({ error: 'Server is currently installing software. Please wait.' });
    }
